...
use std::path::Path;
use thiserror::Error;
use tonsuu_store::{AccuracyStats, HistoryEntry, Store, VehicleStore};
use tonsuu_types::{Error, RegisteredVehicle, TruckClass};
...
impl From<Error> for QueryServiceError {
...
pub fn get_vehicles_by_company(
    ...
    Ok(store
        .all_vehicles()
        .into_iter()
        .filter(|v| v.company.as_ref().map(|c| c.contains(company)).unwrap_or(false))
        .cloned()
        .collect())
}
...
pub fn get_history_with_feedback(
    ...
    let store = open_history_store(config)?;
    let entries: Vec<HistoryEntry> = store.entries_with_feedback().into_iter().cloned().collect();
