...
impl TonsuuApp {
    ...
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        // Open the store via app repository
        let store = open_history_store(&config).unwrap_or_else(|_| {
        ...
        // Open vehicle store via app repository
        let vehicle_store = open_vehicle_store(&config).unwrap_or_else(|_| {
            let fallback_dir = std::env::temp_dir().join("tonsuu-checker-fallback");
