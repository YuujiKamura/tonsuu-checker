...
use crate::extract_json_from_response;
...
/// Plate detection configuration (extracted from Config to avoid circular dependency)
pub struct PlateDetectionConfig {
    pub enabled: bool,
...
pub fn detect_plate_yolo(
    ...
    let cmd_str = match config.command.as_ref() {
    ...
    cmd.arg(image_path);
    cmd.arg("--min-conf");
    cmd.arg(format!("{}", config.min_conf));
    cmd.arg("--output-crop");
    cmd.arg(&crop_path);
