...
use tonsuu_domain::repository::WeighingSlipRepository;
use tonsuu_types::Error;

use crate::csv_loader::load_weighing_slips;
...
impl WeighingSlipRepository for FileWeighingSlipRepository {
    ...
    fn find_by_site(&self, site_name: &str) -> Result<Vec<WeighingSlip>, Error> {
        Ok(self
            .slips
            .iter()
            .filter(|s| s.site_name.as_deref().unwrap_or("").contains(site_name))
            .cloned()
            .collect())
    }
    ...
    fn find_overloaded(&self) -> Result<Vec<WeighingSlip>, Error> {
        Ok(self.slips.iter().filter(|s| s.is_overloaded).cloned().collect())
    }
}
